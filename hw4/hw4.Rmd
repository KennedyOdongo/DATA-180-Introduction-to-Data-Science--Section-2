...
```{r}
records %>% group_by(track) %>% summarise(mean= mean(time), median = median(time), num= n())
```
# Question 2
Return the count of cases where a record was broken in more than a year (365 days).

```{r}
records %>% filter(record_duration>365) %>% summarise(num= n())
```
# Question 3
...
```{r}
records %>% arrange(desc(record_duration)) %>% slice(1) %>% select(player, date, track, record_duration)
```
...
```{r}
records %>% group_by(track) %>% summarise(mean= mean(time)) %>% arrange(desc(mean))
## Rainbow Road
```
# Question 5
...
```{r}
yr <- select(mutate(records,year= substr(date,1, 4)), year)
yr
```
...
```{r}
yr <- records %>% mutate(records, year= substr(date,1, 4)) %>% group_by(year) %>% summarise(mean =mean(time)) %>% select(year, mean)
yr
```
# Question 7
...
```{r, fig.width=10}
library(ggplot2)
records %>% mutate(records, year= substr(date,1, 4)) %>% group_by(year) %>% summarise(mean =mean(time)) %>% select(year, mean) %>% ggplot(aes(year,mean)) + geom_bar(stat='identity')

## The records seem to be generally stable and consistent over time. We can observe a slight slope increase from '97 to '06 and then a similar slight increase from '08 to '19 with two big drops in '07 and '20. Hence there are a few slight trends that could be argued for, but there is no one particular pattern regarding the whole graph. 
```
...
```{r}
records %>% select(type, time) %>% ggplot(., aes(time, fill= type)) + geom_histogram(binwidth = 12 , colour="grey50",alpha=.5)

## A big portion of the three-lap records are located further right on the x (time) axis, simply because it takes more time on average to complete three laps rather than single lap. With that, the majority of single lap records are set within the range up to around 125 seconds.
```
...
```{r}
records %>% mutate(time_per= ifelse(type== "Three Lap",time/3, time)) %>% select(time,time_per, type) %>% ggplot(., aes(time_per, fill= type)) + geom_histogram(binwidth = 12 ,colour="grey50",alpha=.5)
head(records)
```
# Question 10
...
```{r}
records %>% filter(shortcut=="No") %>% mutate(time_per= ifelse(type== "Three Lap",time/3, time))  %>% select(time,time_per, type) %>% ggplot(., aes(time_per, fill= type)) + geom_histogram(binwidth = 12 ,colour="grey50",alpha=.5)
head(records)
```
...
```{r}
records %>% group_by(system_played) %>% summarise(mean= mean(time)) %>% select(mean, system_played)

## We can observe that the average time for records set on the NTSC system (which has more fps and allows players to capture more frames in a second) is lower than that of an average PAL system player (with less frames per second).
```
